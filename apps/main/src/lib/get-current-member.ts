...
export const getCurrentMember = cache(
  async (): Promise<CurrentMember | null> => {
    if (isLocalAdminBypass()) return LOCAL_ADMIN_MEMBER;
    ...
    const { data: member } = await supabase
      .from("members")
      .select(
        "student_id, name, cohort, track, role, status, photo_path, photo_focus",
      )
      .eq("id", user.id)
      .single();
    ...
    return {
      ...
      photoUrl,
      photoFocus:
        (member.photo_focus as PhotoFocus | null) ?? defaultPhotoFocus,
    };
  },
);
